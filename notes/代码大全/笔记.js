...
代价最小捕获错误
P231
这本书的一个目的就是告诉你怎样脱离那种先东拼西凑，然后通过运行来看看代码是否工作的怪圈。
P241
你可能初始化了一个对象的部分成员，而不是全部成员。也可能忘记事先分配内存，就去初始化一个未经初始化的指针所值的“变量”。这就意味着你是随机选取了一块内存，然后对其赋值。这块内存可能存放的数据，也可能存放的是代码。甚至可能指向操作系统内部。指针操作错误可能产生很奇怪的现象，并且每次都不相同 —— 这也导致了调试指针错误比调试其他错误更困难。
